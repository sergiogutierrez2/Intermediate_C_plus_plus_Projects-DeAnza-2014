...
{
    //Make X equal the last element of the vector.
    X = autos.back();
...
{
      if (autos.empty())
      { cout << "There's no car information to display." << endl; }

      else
      {
        for (int count = 0; count < autos.size(); count++)
        {
          cout << endl;
